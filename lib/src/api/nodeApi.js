...
const bts = require("x4tradejs-ws");
const {ChainTypes, ChainStore, FetchChain} = require("x4tradejs");
...
function getAssetData(asset) {
    return new Promise((resolve, reject) => {
        ...
        FetchChain("getObject", asset, undefined, {[asset]: false})
            .then(result => {
                let a = result.toJS();
                assetData[asset] = {
